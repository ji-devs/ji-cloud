    ...
    pub async fn spawn_cron_jobs(&self) -> anyhow::Result<()> {
        ...
        for count in 0..2 {
            let res = if count == 0 {
                ...
                self.update_jig_translations().await.context("update jig description translation task errored")
            };
...
limit 10 for no key update skip locked;
 "#
        ...
        log::debug!("Updating a batch of {} image description(s)", requests.len());

        for t in requests {
            let translated_description: Option<Value> = multi_translation(&t.description, &self.api_key).await?;
...
select jig_data.id,
       description                                                                                    
from jig_data
     join jig on jig.draft_id = jig_data.id
where description <> '' and translated_description = '{}'
...
pub async fn translate_text(
    ...
    // log::warn!("query: {}, {}, {}", query, target, source);
...
pub async fn test_multi_translations(description: &str, api_key: &str) -> anyhow::Result<()> {
    //https://cloud.google.com/translate/docs/languages
    //https://cloud.google.com/translate/docs/reference/rest/v2/translate
    let res = reqwest::Client::new()
        .post("https://translation.googleapis.com/language/translate/v2/detect")
    ...
    println!("translation object: {}", list_json);

    Ok(())
}

#[actix_rt::test]
async fn detection_test() -> anyhow::Result<()> {
    const KEY: &str = "AIzaSyCJ4t4OF3WDdA9AFJ25SFiyR1gy9ElSkLw";

    test_multi_translations("משחק לגן דרור (Originally created on Ji Tap by לילי )", KEY).await?;
