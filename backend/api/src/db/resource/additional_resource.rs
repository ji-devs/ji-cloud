...
pub async fn get(
    ...
    let (draft_id, live_id) = super::get_draft_and_live_ids(&mut txn, resource_id)
        .await
        .ok_or(error::NotFound::ResourceNotFound)?;

    let resource_data_id = match draft_or_live {
...
pub async fn update(
    ...
    let (draft_id, live_id) = super::get_draft_and_live_ids(&mut txn, resource_id)
        .await
        .ok_or(anyhow::anyhow!("failed to get resource_data IDs"))?;
...
where (resource_data_id = $1 or resource_data_id = $2)
  and id = $3
        "#,
        draft_id,
        live_id,
