...
use shared::domain::jig::module::{ModuleBody, ModuleKind, ModuleUpdateRequest};
...
async fn update_contents() -> anyhow::Result<()> {
    ...
    let resp = client
        ...
        .login()
        .json(&ModuleUpdateRequest{ body: Some(ModuleBody::Unknown {
            body: json!({}),
...
#[actix_rt::test]
async fn update_kind() -> anyhow::Result<()> {
    let app = initialize_server(&[Fixture::User, Fixture::Jig]).await;
