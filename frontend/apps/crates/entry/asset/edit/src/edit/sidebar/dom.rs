            ...
            .child(html!("jig-edit-sidebar", {
                ...
                .future(clone!(state => async move {
                    state.asset_edit_state.route.signal_cloned().for_each(clone!(state => move |route| {
                        let should_collapse = !matches!(
                            route,
                            AssetEditRoute::Jig(_, JigEditRoute::Landing) | AssetEditRoute::Playlist(_, PlaylistEditRoute::Landing) | AssetEditRoute::Course(_, CourseEditRoute::Landing) | AssetEditRoute::Course(_, CourseEditRoute::Unit(_))
                        );
                ...
                .child(html!("jig-edit-sidebar-publish", {
                    ...
                    .prop_signal("selected", state.asset_edit_state.route.signal_cloned().map(|route| {
                        matches!(
                            route,
                            AssetEditRoute::Jig(_, JigEditRoute::Publish) | AssetEditRoute::Playlist(_, PlaylistEditRoute::Publish) | AssetEditRoute::Course(_, CourseEditRoute::Publish)
                        )
                    }))
                    .event(clone!(state => move |_ :events::Click| {
                        if state.can_publish() {
                            ...
                            match state.asset_edit_state.asset_id {
                                ...
                                shared::domain::asset::AssetId::CourseId(_) => actions::set_highlight_units(&state, true),
                            }
                        }
                    }))
