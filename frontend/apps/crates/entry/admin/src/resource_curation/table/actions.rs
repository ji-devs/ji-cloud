...
use crate::resource_curation::FetchMode;

use super::state::ResourceTable;

impl ResourceTable {
    pub fn search_resources(self: &Rc<Self>, query: String) {
        let state = self;
        ...
        state.loader.load(clone!(state => async move {
            state.curation_state.load_resources().await;
        }));
    }
}
