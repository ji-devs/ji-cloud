        ...
        html!("input-select", {
            ...
            .prop_signal("error", {
                (map_ref! {
                    ...
                    let value = state.resource.categories.signal_cloned()
    ...
    categories.iter().map(|category| {
        if category.children.len() == 0 {
            ...
            html!("input-select-option", {
                .text(&category.name)
                .prop_signal("selected", state.resource.categories.signal_cloned().map(clone!(category_id => move |selected_categories| {
                    selected_categories.contains(&category_id)
                })))
                .event(clone!(state => move |_: events::CustomSelectedChange| {
                    let mut categories = state.resource.categories.lock_mut();
...
fn category_value_signal(state: Rc<Publish>) -> impl Signal<Item = String> {
    map_ref! {
        let selected_categories = state.resource.categories.signal_cloned(),
