...
impl ResourceDetails {
    pub fn render_ages(self: &Rc<Self>) -> Dom {
        ...
        html!("input-select", {
            ...
            //         let value = state.resource.age_ranges.signal_cloned()
...
fn render_age(age: &AgeRange, state: Rc<ResourceDetails>) -> Dom {
    let age_id = age.id;
    html!("input-select-option", {
        .text(&age.display_name)
        .prop_signal("selected", state.resource.age_ranges.signal_cloned().map(clone!(age_id => move |ages| {
            ages.contains(&age_id)
        })))
        .event(clone!(state => move |_: events::CustomSelectedChange| {
            let mut ages = state.resource.age_ranges.lock_mut();
...
fn age_value_signal(state: Rc<ResourceDetails>) -> impl Signal<Item = String> {
    map_ref! {
        let selected_ages = state.resource.age_ranges.signal_cloned(),
