...
impl ResourceDetails {
    pub fn render_affiliations(self: &Rc<Self>) -> Dom {
        ...
        html!("input-select", {
            ...
            //         let value = state.resource.affiliation_ranges.signal_cloned()
...
fn render_affiliation(affiliation: &Affiliation, state: Rc<ResourceDetails>) -> Dom {
    let affiliation_id = affiliation.id;
    html!("input-select-option", {
        .text(&affiliation.display_name)
        .prop_signal("selected", state.resource.affiliations.signal_cloned().map(clone!(affiliation_id => move |affiliations| {
            affiliations.contains(&affiliation_id)
        })))
        .event(clone!(state => move |_: events::CustomSelectedChange| {
            let mut affiliations = state.resource.affiliations.lock_mut();
...
fn affiliation_value_signal(state: Rc<ResourceDetails>) -> impl Signal<Item = String> {
    map_ref! {
        let selected_affiliations = state.resource.affiliations.signal_cloned(),
