...
impl Section3Dom {
    pub fn render(meta_state: Rc<MetaState>, image: Rc<MutableImage>, metadata: Rc<MetadataResponse>, categories: Rc<Vec<Rc<MutableCategory>>>, tag_list: Rc<Vec<(ImageTag, TagId)>>) -> Dom {
        let state = Rc::new(State::new(meta_state, image, metadata, categories, tag_list));
