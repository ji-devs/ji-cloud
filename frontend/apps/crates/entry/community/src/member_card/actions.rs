...
pub fn follow_user(member_id: UserId) {
    spawn_local(async move {
        endpoints::user::Follow::api_with_auth_empty(PublicUserFollowPath(member_id), None)
            .await
            .unwrap_ji();
    });
}
...
pub fn unfollow_user(member_id: UserId) {
    spawn_local(async move {
        endpoints::user::Unfollow::api_with_auth_empty(PublicUserUnfollowPath(member_id), None)
            .await
            .unwrap_ji();
    })
}
