...
use shared::{api::{ApiEndpoint, endpoints}, domain::jig::AudioBackground, error::EmptyError};
...
pub fn on_background_audio_click(state: Rc<State>, audio_background: AudioBackground) {
    ...
    *selected_audio = match &*selected_audio {
        Some(selected_audio) if selected_audio == &audio_background => {
            None
        },
        _ => {
...
pub fn update_jig_settings(state: Rc<State>) {
    ...
    let path = endpoints::jig::Update::PATH
        .replace("{id}", &state.jig_id.0.to_string());
