...
use shared::domain::course::{AudioFeedbackNegative, AudioFeedbackPositive};
use utils::{events, course::CourseAudioExt};
...
where
    T: Hash + Eq + Clone + CourseAudioExt + Into<AudioPath<'a>> + std::fmt::Debug + 'static,
{
    let audio_handle = &audio_handles[index];

    html!("course-audio-line", {
        .property("slot", "lines")
