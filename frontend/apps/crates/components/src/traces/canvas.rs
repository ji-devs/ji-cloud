...
use utils::{math::{quat, BoundsF64, mat4::Matrix4}, prelude::*, resize::ResizeInfo};
use super::utils::*;
...
pub fn apply_transform_mat4(ctx:&CanvasRenderingContext2d, mat: &Matrix4) {
    ...
    let _ = ctx.transform(scale_x, skew_y, skew_x, scale_y, translate_x, translate_y).unwrap_ji();
}

//this was written before the above apply_transform_mat4
//it might be able to be simplified by just building on that
pub fn apply_transform(ctx: &CanvasRenderingContext2d, resize_info: &ResizeInfo, transform: &Transform, bounds: &BoundsF64) {
...
pub fn clip_single_shape(ctx: &CanvasRenderingContext2d, resize_info: &ResizeInfo, shape: &TraceShape) {
