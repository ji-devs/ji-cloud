use super::{EndPolicy, state::TraceBubble};
...
impl TraceBubble {
    ...
    pub fn reset_ended(&self) {
        self.fade_ended.store(self.tooltip.is_none(), Ordering::SeqCst);
        self.audio_ended.store(self.audio.is_none(), Ordering::SeqCst);
    }
