    ...
    S: Signal<Item = ShapeStyle> + 'static
{
    ...
    pub fn apply(self, dom:DomBuilder<SvgElement>) -> DomBuilder<SvgElement> {
        match self {
            ShapeStyleVar::Static(shape_style) => {
                shape_style.apply(dom)
            },
            ShapeStyleVar::Dynamic(shape_style_signal) => {
                dom.attr_signal("class", shape_style_signal.map(|shape_style| {
                    shape_style.classes_string()
                }))
            }
        }
    }
...
impl ShapeStyle {
    pub fn apply(&self, dom:DomBuilder<SvgElement>) -> DomBuilder<SvgElement> {
        self.classes().iter().fold(dom, |dom, class_name| {
            dom.class(class_name)
        })
    }
}
