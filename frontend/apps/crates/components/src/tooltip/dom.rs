    ...
    pub fn render(data: TooltipData) -> Dom {
        match data {
            TooltipData::Error(data) => {
                let TooltipError {move_strategy, elem, placement, slot, body, on_close, max_width } = data;
                html!("tooltip-error", {
                    .text(&body)
            ...
            TooltipData::Confirm(data) => {
                let TooltipConfirm {move_strategy, elem, placement, slot, header, confirm_label, max_width, cancel_label, on_confirm, on_cancel} = data;
