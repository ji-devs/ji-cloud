/* This all technically compiles... but it crashes :/ See example usage at the bottom
 */
 ...
 *    is provided as a helper. It does not prevent dynamic dispatch, however
...
where
    ...
    Data: DeserializeOwned + std::fmt::Debug + 'static,
{
    ...
    fn page_kind_signal(_self: Rc<Self>) -> impl Signal<Item = Option<ModulePageKind>> {
        _self.has_loaded_data.signal().map(clone!(_self => move |has_loaded| {
            if has_loaded {
                ...
                OptionModulePageKindSignal::new(Some(
                        Renderer::page_kind_signal(renderer.clone())
                ))
            } else {
                OptionModulePageKindSignal::new(None)
            }
        }))
        .flatten()
    }

    fn sidebar_signal(_self: Rc<Self>) -> impl Signal<Item = Option<Dom>> {
        _self.has_loaded_data.signal().map(clone!(_self => move |has_loaded| {
            if has_loaded {
                let renderer = _self.module_renderer.borrow();
                let renderer = renderer.as_ref().unwrap_throw();
                OptionDynDomSignal::new(Some(
                        Renderer::sidebar_signal(renderer.clone())
                ))
            } else {
                OptionDynDomSignal::new(None)
            }
        }))
        .flatten()
    }
    fn header_signal(_self: Rc<Self>) -> impl Signal<Item = Option<Dom>> {
        _self.has_loaded_data.signal().map(clone!(_self => move |has_loaded| {
            if has_loaded {
                let renderer = _self.module_renderer.borrow();
                let renderer = renderer.as_ref().unwrap_throw();
    ...
    fn main_signal(_self: Rc<Self>) -> impl Signal<Item = Option<Dom>> {
        _self.has_loaded_data.signal().map(clone!(_self => move |has_loaded| {
            if has_loaded {
                let renderer = _self.module_renderer.borrow();
                let renderer = renderer.as_ref().unwrap_throw();
    ...
    fn footer_signal(_self: Rc<Self>) -> impl Signal<Item = Option<Dom>> {
        _self.has_loaded_data.signal().map(clone!(_self => move |has_loaded| {
            if has_loaded {
                let renderer = _self.module_renderer.borrow();
                let renderer = renderer.as_ref().unwrap_throw();
    ...
    fn dom_signal(_self: Rc<Self>) -> impl Signal<Item = Option<Dom>> {
        Self::page_kind_signal(_self.clone()).map(clone!(_self => move |page_kind| {
            page_kind.map(|page_kind| {
                elem!(templates::module_page(page_kind), {
                    .with_data_id!("sidebar", { .child_signal( 
                        Self::sidebar_signal(_self.clone())
                    )})
...
//ModulePageKind
//Ok fitting with the pattern below this is really OptionOptionDomSignal... but w/e

type ModulePageKindSignal = Pin<Box<dyn Signal<Item = ModulePageKind>>>;

pub struct OptionModulePageKindSignal {
    value_signal: Option<ModulePageKindSignal>, 
    const_has_fired: bool
}

impl OptionModulePageKindSignal {
    pub fn new(value_signal: Option<ModulePageKindSignal>) -> Self {
        Self {
            value_signal,
...
impl Signal for OptionModulePageKindSignal {
    type Item = Option<ModulePageKind>;
...
type DynDomSignal = Pin<Box<dyn Signal<Item = Option<Dom>>>>;

pub struct OptionDynDomSignal {
    value_signal: Option<DynDomSignal>, 
    const_has_fired: bool
}

impl OptionDynDomSignal {
    pub fn new(value_signal: Option<DynDomSignal>) -> Self {
        Self {
            value_signal,
            const_has_fired: false,
        }
...
impl Signal for OptionDynDomSignal {
    type Item = Option<Dom>;
