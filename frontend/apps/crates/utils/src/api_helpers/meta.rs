use shared::domain::meta::{MetadataResponse, MetaKind};
...
impl MetaOptions {
    pub async fn load() -> Result<Self, ()> {
        ...
        let resp:Result<MetadataResponse, ()> = api_no_auth::<_, _, ()>(&endpoints::meta::Get::PATH, endpoints::meta::Get::METHOD, None).await;
        resp
            .map_err(|err| {
                //log::error!("{:?}", err);
                ()
