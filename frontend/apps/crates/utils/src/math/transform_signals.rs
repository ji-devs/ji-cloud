...
pub fn map_offset(
    transform: Transform,
    offset: impl Signal<Item = (f64, f64)> + 'static,
) -> impl Signal<Item = Transform> {
    offset.map(move |(offset_x, offset_y)| transform.map_offset(offset_x, offset_y))
}
/*
...
pub fn x_px(
    ...
    aabb_no_rotation_bounds_px(coords_in_center, transform_signal, size_signal)
        .map(|bounds| bounds.x)
}
...
pub fn denormalize_matrix_string(
    transform_signal: impl Signal<Item = Transform>,
...
pub fn scale_matrix_string(
    transform_signal: impl Signal<Item = Transform>,
) -> impl Signal<Item = String> {
    transform_signal.map(|t| t.scale_matrix_string())
}
//CSS requires the full 4x4 or 6-element 2d matrix, so we return the whole thing
//but set the scale and translation to identity
pub fn rotation_matrix_string(
    transform_signal: impl Signal<Item = Transform>,
) -> impl Signal<Item = String> {
    transform_signal.map(|t| t.rotation_matrix_string())
}
pub fn invert_rotation_matrix_string(
    transform_signal: impl Signal<Item = Transform>,
) -> impl Signal<Item = String> {
    transform_signal.map(|t| t.invert_rotation_matrix_string())
}
