...
import { ContainerMode } from "@elements/core/inputs/composed/dropdown-tree";
...
export const DropdownTree = (props?: Partial<Args>) => {
    ...
    const containerMode = mock === "categories" ? "multi-color" : "none";
    ...
    const getContent = ({
        ...
        let content = (() => {
            switch (mode) {
                case "checkbox":
                    return `<input-checkbox slot="content" label="${label}"></input-checkbox>`;
                case "textDisplay":
                    return `<span slot="content">${label}</span>`;
                case "textInput":
                    return `<input slot="content" style="text" value="${label}" />`;
                default:
                    return ``;
            }
        })();
    ...
    const renderMenu = (content: any, menuContents: boolean) => {
        return `<menu-ellipses slot="content" ${
            menuContents ? "visible" : ""
        } hover>
        ${content}
