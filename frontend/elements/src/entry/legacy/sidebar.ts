...
function makeClosed(dispatcher: LitElement) {
    return html`
        <div
            class="open"
            @click="${() => dispatcher.dispatchEvent(new Event("open"))}"
        >
            -->
        </div>
    `;
...
function makeOpen(dispatcher: LitElement, nModules: number) {
    return html`
        <aside>
            <div
                class="close"
                @click="${() => dispatcher.dispatchEvent(new Event("close"))}"
            >
                <-- close
            </div>
            <div class="modules">
                ${arrayIndex(nModules).map((index) =>
                    moduleSlot(index, nModules)
                )}
            </div>
        </aside>
    `;
...
const moduleSlot = (index: number, max: number) => html`
    ...
    ${index !== max ? html`<div class="separator"></div>` : nothing}
`;
