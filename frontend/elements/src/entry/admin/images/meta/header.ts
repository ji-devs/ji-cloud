...
export class _ extends LitElement {
	...
	const renderTab = (target:SECTION, decorate:boolean) => {
		return html`
			<div class=${classMap({tab: true, tabSelected: target === section})} @click=${() => this.gotoRoute(target)}>
        ...
        return html`
			...
			<div class="tabs">
                ...
                <div class="right">
                    ...
                    <button-rect @click=${() => this.gotoRoute("publish")} color="orange" size="small">${STR_PUBLISH}</button-rect>
                </div>
            </aside>
  `;
    }
