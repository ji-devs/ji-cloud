...
type BooleanControlKey = Extract<
    {
        [key in keyof ControllerState]: ControllerState[key] extends boolean
            ? key
            : never;
    }[keyof ControllerState],
    string
>;
...
export class EditorBackbone {
    ...
    public isMarkActive(key: BooleanControlKey): boolean {
        const iterators = Editor.nodes(this.editor, {
    ...
    getValue<K extends keyof ControllerState>(
        key: K
    ): ControllerState[K] | undefined {
    ...
    setValue<K extends keyof ControllerState>(
        key: K,
